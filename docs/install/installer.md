...
:::tip 가장 중요한 다음 단계
설치가 끝나면 바로 아래 명령어를 이어서 실행하세요.
```bash
openclaw onboard --install-daemon
```
이 명령이 모델 연결, 채널 연결, 데몬 설치를 한 번에 이어줍니다.
:::
...
Node.js가 이미 준비된 환경에서는 아래 방법도 사용할 수 있습니다.
...
:::tip 이 명령부터 꼭 기억하세요
설치만 하고 멈추지 말고, **`openclaw onboard --install-daemon`**까지 이어서 실행해야 실제로 쓰기 쉬운 상태가 됩니다.
:::
