...
|---|---|
| 현재 단계 | **Beginner** |
| 읽는 목적 | 내 환경에 맞는 설치 경로를 먼저 고르기 |
| 추천 환경 | **Windows + WSL2 + Ubuntu** 또는 macOS / Linux 터미널 |
| 현재 위치 | `install/index` |
...
| 내 상황 | 추천 경로 | 이유 |
|---|---|---|
| 빠르게 시작하고 싶다 | npm 설치 + 온보딩 | 가장 공식적인 기본 경로 |
...
이 문서는 **내 환경에서 어떤 설치 경로를 고를지 빠르게 결정하는 안내판**입니다. 대부분의 사용자는 npm 한 줄 설치로 끝나지만, 팀 운영·컨테이너·선언적 배포처럼 상황이 달라지면 선택도 달라집니다.
...
| 방법 | 추천 대상 |
|------|---------|
| **설치 스크립트** | 도구 준비가 덜 된 환경, 스크립트 제공 시 |
| **설치 스크립트 (공식 권장)** | 가장 빨리 설치하고 싶은 환경, 도구가 아직 덜 준비된 환경 |
| **npm 직접 설치 (대안)** | Node.js가 이미 설치된 환경 |
...
### macOS / Linux / Windows WSL2 — 공식 권장
...
:::tip 💡 언제 npm을 쓰나요?
컴퓨터에 Node.js가 이미 준비되어 있다면 이 방법도 잘 작동합니다.
하지만 처음 설치라면 **설치 스크립트 + 온보딩** 흐름이 가장 쉽습니다.
:::
...
:::tip 🌟 가장 중요한 한 줄
OpenClaw 설치에서 제일 먼저 기억할 것은 **`openclaw onboard --install-daemon`** 입니다.
설치가 끝나면 이 명령으로 **모델 연결 + 채널 연결 + 데몬 설치**를 한 번에 이어갈 수 있어요.
:::

## 🔧 npm 설치 중 sharp 에러가 날 때

가끔 `npm install -g openclaw@latest` 중에 **`sharp` 설치 에러**가 날 수 있습니다.

쉽게 말하면, 컴퓨터 안에 있는 이미지 처리 부품(`libvips`)과 npm이 내려받으려는 부품이 서로 헷갈릴 때 생기는 문제입니다.
...
이 명령은 **컴퓨터에 이미 깔린 전역 libvips를 무시하고**, sharp가 자기 기준으로 다시 설치하게 도와줍니다.

:::tip 언제 이 방법을 쓰나요?
- `sharp` 관련 오류가 보일 때
- Linux/WSL2에서 예전에 다른 방식으로 libvips를 설치한 적이 있을 때
- npm 글로벌 설치가 중간에 멈출 때
:::

:::caution 무조건 먼저 쓸 필요는 없습니다
평소에는 일반 설치 명령으로 먼저 시도하세요.
`sharp` 에러가 실제로 보일 때만 이 환경 변수를 붙이면 됩니다.
:::
