...
| 항목 | 요구사항 |
|------|---------|
| **Node.js** | Node 24 권장, Node 22.14+ 호환 |
| **운영체제** | macOS, Linux, Windows (WSL2 권장) |
| **메모리** | 최소 4GB RAM 권장 |

```bash
node --version
# v24.x.x 또는 v22.14+ 이상
```
...
| **npm 직접 설치 (공식 권장)** | Node.js가 설치된 모든 환경 |
...
### 대안 — 설치 스크립트 (스크립트가 제공되는 경우)

```bash
# 스크립트로 설치 (Node.js, Git 등 자동 설치 지원)
...
:::tip 💡 스크립트는 언제 쓰나요?
`install.sh`는 Node.js가 아직 없는 환경에서 Homebrew, Git, Node.js를 자동 설치하려고 시도합니다.
단, **스크립트가 실제로 제공되는지는 공식 사이트에서 확인**하고 사용하세요.
:::
