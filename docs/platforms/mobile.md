...
- 모바일 기기와 Gateway가 같은 네트워크에 있거나, Tailscale 같은 안전한 원격 연결이 준비되어 있어야 합니다.
- Gateway가 켜진 컴퓨터에서 `openclaw` 명령어를 실행할 수 있어야 합니다.

:::tip 먼저 이렇게 생각하면 쉬워요
모바일 앱이 먼저 Gateway에 "연결해도 될까요?" 하고 손을 듭니다.
그러면 Gateway 쪽에서 **승인 버튼을 눌러 주는 흐름**이라고 생각하면 됩니다.
:::
...
2. 모바일 앱에서 Gateway를 선택하거나 직접 주소를 입력합니다.
   - iPhone/iPad는 **QR 코드** 또는 **설정 코드**를 쓸 수 있어요.
   - Android는 **Setup Code** 또는 직접 주소 입력 방식이 있어요.

3. Gateway가 받은 페어링 요청을 확인합니다.
   ```bash
   openclaw devices list
   ```

4. 화면에 보이는 `requestId`를 승인합니다.
   ```bash
...
- `openclaw devices list`에서 **pending 요청**이 보여요.
- `openclaw devices approve <requestId>`를 실행하면 승인이 돼요.
- `openclaw nodes status`에서 내 모바일 기기가 보여요.

:::tip iPhone/iPad에서 QR 코드가 필요하다면
아래 명령어로 iOS용 QR 코드나 설정 코드를 만들 수 있어요.

```bash
openclaw qr
```

설정 코드만 보고 싶다면 이렇게 입력하세요.
