<!-- source: https://openclaw-wheat.vercel.app/gateway/ -->
...
### 📡 원격 접속 및 활성화 (/gateway/remote)​ (#-원격-접속-및-활성화)

로컬에 설치된 게이트웨이에 모바일 앱이나 외부 기기로부터 안전하게 연결하는 방법을 설명합니다.

### 🛡️ 보안 강화 (Security Audit) (/gateway/security)​ (#️-보안-강화-security-audit)

인증 정보 유출 방지 및 시스템 권한 점검을 통해 안전한 운영 환경을 구축합니다.

### 🐳 샌드박스 운영 (/gateway/sandboxing)​ (#-샌드박스-운영)

에이전트의 도구 실행을 Docker 등으로 완벽하게 격리하여 서버를 보호합니다.

### 🔌 API 및 프로토콜 (/gateway/api)​ (#-api-및-프로토콜)

REST API 및 MCP(Model Context Protocol)를 통해 외부 서비스를 연동하는 기술 스펙을 다룹니다.

## ⚙️ 런타임 갱신 (Hot-reload)​ (#️-런타임-갱신-hot-reload)
...
- 🚀 주요 운영 섹션 (#-주요-운영-섹션)
- 📡 원격 접속 및 활성화 (#-원격-접속-및-활성화)

- 🛡️ 보안 강화 (Security Audit) (#️-보안-강화-security-audit)
